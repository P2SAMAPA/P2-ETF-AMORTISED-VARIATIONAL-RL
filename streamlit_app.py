...
st.set_page_config(page_title="Amortised Variational RL (PEARL/VariBAD)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🤖 Amortised Variational RL Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">PEARL / VariBAD – Amortised context inference for regime-adaptive policy</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧬 Amortised VI")
...
st.sidebar.markdown(f"**Latent dim:** {config.LATENT_DIM} | **Context size:** {config.CONTEXT_SIZE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'amortised_rl_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>AVI score: {etf['avi_score_norm']:.3f}</p>
...
with tab1:
    st.header("🤖 Top ETFs by Amortised Variational RL Score (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Amortised variational inference** for meta-RL (PEARL / VariBAD) learns to infer a latent variable representing the current market regime.
        - The encoder takes a short history of returns and outputs a distribution over latent `z`.
        - The policy conditions on `z` and recent state to predict next-day return.
        - At inference time, the agent adapts instantly by inferring `z` from the most recent context.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the amortised variational RL scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Amortised Variational RL | PEARL / VariBAD – regime inference for ETF ranking")
